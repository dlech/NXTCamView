...
namespace NXTCamView.Forms
{
    public partial class ColorForm : Form
    {
        ...
        private const int COLOR_DETAIL_HEIGHT = 241;
        private const int COLOR_PANEL_SPACING = 30;
        public const int TrackedColors = 8;
        private readonly ColorDetail _colorDetail = new ColorDetail();
        private readonly Panel[] _colorPanels = new Panel[TrackedColors];
        private readonly OverlapInfo[] _overlapInfos = new OverlapInfo[TrackedColors];
        private static Image _errorImage;
        private int _selectedColorIndex;
        private bool _isColorDetailVisible;
        ...
        private IColorTarget _colorTarget;
        private ICommsPort _commsPort;
        ...
        public MainForm MainForm
        {
            ...
            set { 
                if( _mainForm != null )
                {
                    _mainForm.ConnectionStateChanged -= mainForm_ConnectionStateChanged;
                }
                _mainForm = value;
                _mainForm.ConnectionStateChanged += mainForm_ConnectionStateChanged;
            }
        ...
        private void ColorForm_Load(object sender, EventArgs e)
        {
            ...
            _colorDetail.ColorChanged += _colorDetail_ColorChanged;
            _colorDetail.Refresh();

            Settings settings = Settings.Default;
            _selectedColorIndex = settings.SelectedColorIndex;

            for (int index = 0; index < TrackedColors; index++)
            {
                ...
                panel.Left = pnlSample.Left + (COLOR_PANEL_SPACING * index) - ((TrackedColors - 1) * COLOR_PANEL_SPACING);
                panel.BackColor = settings.GetAverageColor(index);
                panel.Tag = index;
                panel.Click += panel_Click;
                panel.MyPaint += panel_MyPaint;
        ...
        {
            setupItem(tsbSetColor, "Set color", "Use color setting mode", AppImages.SetColor, new SetColorFunctionModeStripCommand(_appState, this, ColorFunction.SetColor));
            setupItem(tsbAddToColor, "Add to color", "Use color adding mode (or press CTRL while clicking)", AppImages.AddToColor, new SetColorFunctionModeStripCommand(_appState, this, ColorFunction.AddToColor));
            setupItem(tsbRemoveFromColor, "Remove from color", "Use color removing mode (or press CTRL+SHIFT while clicking)", AppImages.RemoveFromColor, new SetColorFunctionModeStripCommand(_appState,this, ColorFunction.RemoveFromColor));

            setupItem(tsbUpload, "Upload", "Upload tracking colors to NXTCam", AppImages.UploadColors, new UploadColorsStripCommand( _appState, this, _colorTarget, _commsPort ));
            setupItem(tsbClear, "Clear", "Clear color", AppImages.ClearColor, new ClearColorsStripCommand(_appState, false, this));
            setupItem(tsbClearAll, "Clear All", "Clear all colors", AppImages.ClearAllColors, new ClearColorsStripCommand(_appState, true, this));
            _highlightColorCmd = new HighlightColorStripCommand(_appState, true, this);
            setupItem(tsbHighlight, "Highlight", "Highlight the color in the capture", AppImages.HighlightColor, _highlightColorCmd);
            setupItem(tslShowHide, "Hide", "Show/Hide colors", null, new ToggleShowHideStripCommand(_appState, this));
        }

        private void positionTopRight()
        {
            MdiClient mdiClient = StickyWindow.GetMdiClient(_mainForm);
            if (mdiClient != null)
            {
                Top = 0;
        ...
        void pnlColorPanels_Paint(object sender, PaintEventArgs e)
        {
            ...
            rect.X = pnlSample.Left + (COLOR_PANEL_SPACING * _selectedColorIndex) - ((TrackedColors - 1) * COLOR_PANEL_SPACING) - 2;
            rect.Y -= 2;
            rect.Width += 3;
            rect.Height += 3;
        ...
        {
            updateOverlapStatus(index, TrackedColors, Settings.Default.MinColors[index], Settings.Default.MaxColors[index]);
        }
        ...
        {
            private readonly ColorForm _form;
