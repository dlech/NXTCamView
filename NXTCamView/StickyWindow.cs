...
//
//Paul Tingey 2007 - Mods have been added to enable MDI windows and enable disable the all window
//
using System;
...
namespace Blue.Windows
{
    ...
    public class StickyWindow : System.Windows.Forms.NativeWindow
    {
        ...
        private static MdiClient mdiClient = null;
        ...
        private static bool globalActive = true;
        ...
        /// <summary>
        /// Enable/disable all stick windows
        /// Default value = true
        /// </summary>
        public static bool Active { get { return globalActive; } set { globalActive = value; } }
        ...
        public StickyWindow(Form form)
        {
            ...
            originalForm.HandleCreated += new EventHandler(originalForm_HandleCreated);
            originalForm.HandleDestroyed += new EventHandler(originalForm_HandleDestroyed);
        }
        #endregion
        ...
        {
            //if the active state is toggled globally, then cancel any changes for this window
            if( globalActive != active )
            {
                Cancel();
                active = globalActive;
            }
            if (!globalActive || !MessageProcessor(ref m))
                base.WndProc(ref m);
        }
        #endregion
        ...
        {
            if (originalForm.IsMdiChild)
            {
                MdiClient mdiClient = GetMdiClient(originalForm.MdiParent);
                p = mdiClient.PointToClient(originalForm.PointToScreen(p));
            }
            else
            {
            ...
            if (stickToScreen && !originalForm.IsMdiChild)
            ...
            if (stickToOther)
            {
                ...
                {
                    Rectangle swRect = GetStickyWindowRect(sw);
                    if( swRect != Rectangle.Empty ) Resize_Stick(swRect, true);
                }
            }
        ...
        private void Move(Point p)
        {
            ...
            if (stickToOther)
            {
                ...
                {
                    Rectangle swRect = GetStickyWindowRect(sw);
                    if( swRect != Rectangle.Empty ) Move_Stick(swRect, true);
                }
            }
        ...
        private Rectangle GetStickyWindowRect(Form sw)
        {
            Rectangle swRect = Rectangle.Empty;
            if (sw != originalForm)
            {
                if (originalForm.IsMdiChild && originalForm.MdiParent == sw)
                {
                    MdiClient mdiClient = GetMdiClient(sw);
                    if (mdiClient != null)
                    {
                        swRect = new Rectangle(Point.Empty, mdiClient.ClientSize);
                    }
                }
                else
                {
        ...
        private void originalForm_HandleCreated(object sender, EventArgs e)
        {
            AssignHandle(originalForm.Handle);
        }

        private void originalForm_HandleDestroyed(object sender, EventArgs e)
        {
            ReleaseHandle();
        }
        #endregion
